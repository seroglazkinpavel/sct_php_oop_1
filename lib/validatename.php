...
class ValidateName extends Validator
{
    ...
    public function validate(): array
    {
        ...
        if (isset($req['name'])) {
            ...
            } elseif ($this->isContainQuotes($req['name'])) {
                $this->addError(self::CODE_INVALID);// если строка содержит кавычки мы ошибку ставим и наобарот
            } elseif (!preg_match("/^([a-zA-Z' -]|[а-яА-ЯЁёІіЇїҐґЄє' -])*$/u", $req['name'])) {
